...
import { RANK_DEFS_CIV6, type RankNames } from '../../core/config/constants.js';
...
export function pickRankName(mu: number): RankNames {
...
export function uniqueRatings(input: readonly AffectedPlayerRating[]): AffectedPlayerRating[] {
...
export async function updateRankRolesForApprovedMatch(
  ...
  if (!canManage) {
    logWarn(`[rank-roles] Missing ManageRoles permission: guild=${guild.id}`);
    return;
  }

  const { byName, managedIds, missingConfig, missingInGuild } = collectManagedRoles(guild);
  if (missingConfig.length > 0) {
    logWarn(`[rank-roles] Missing rank role IDs in config: ${missingConfig.join(', ')}`);
  }
  if (missingInGuild.length > 0) {
    logWarn(`[rank-roles] Rank roles not found in guild: ${missingInGuild.join(', ')}`);
  }
  ...
  await runBounded(unique, opts.concurrency ?? 3, async ({ discord_id, rating_mu }) => {
    ...
    if (!member) {
      logWarn(`[rank-roles] Member not found: guild=${guild.id} member=${discord_id}`);
      return;
    }
