...
import { randomIndex, shuffleInPlace } from '../random.js';

const MULTI_VALUE_DELIMITER = '|';

export type VoteRecord = ReadonlyMap<string, string>;
...
export function pickRandomVoteValue(question: VoteQuestion, rng: RandomSource): string {
  if (!isMultiSelectQuestion(question)) {
    return (question.options[randomIndex(rng, question.options.length)] as VoteQuestion['options'][number]).id;
  }
  ...
  const count = Math.max(
    ...
    Math.min(
      ...
      1 + Math.floor(rng() * getQuestionMaxSelections(question))
    )
  );
  const pool = question.options.map((option) => option.id);
  shuffleInPlace(pool, rng);

  // count >= 1 and every id comes from question.options, so the encode never returns null
  return encodeVoteSelections(question, pool.slice(0, count)) as string;
}
