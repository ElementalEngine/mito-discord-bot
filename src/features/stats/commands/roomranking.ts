...
import { formatRoomRanksPages } from '../ui/roomranks.layout.js';
import type {
  RoomRanksLifetimeRow,
  RoomRanksRow,
} from '../ui/roomranks.layout.js';
...
export async function execute(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  if (!(interaction.member instanceof GuildMember)) {
    await replyEphemeral(interaction, 'Member context is missing for this interaction.');
    return;
  }

  const voiceChannel = interaction.member.voice.channel;
  if (!voiceChannel || !('members' in voiceChannel)) {
    await replyEphemeral(interaction, 'Join a voice channel first, then run /roomranks.');
    return;
  }

  const members = [...voiceChannel.members.values()].filter((m) => !m.user.bot);
  if (members.length === 0) {
    await replyEphemeral(interaction, 'No non-bot users found in your voice channel.');
    return;
  }
