...
import type { LeaderboardRanking } from '../../../core/api/types.js';
import type { Leaderboard } from '../../../core/config/leaderboards.config.js';
import { leaderboardsList } from '../../../core/config/leaderboards.config.js';
...
/** @internal — exported for tests; pure string formatting. */
...
/** @internal — exported for tests; pure parsing. */
export function isLeaderboardUpToDate(
  leaderboardRanking: LeaderboardRanking,
...
async function updateLeaderboard(
  ...
  const leaderboardRanking = await statsService.getLeaderboardRanking({
    game: leaderboard.game,
    gameType: leaderboard.game_type,
    gameMode: leaderboard.game_mode,
    isSeasonal: leaderboard.is_seasonal,
    isCombined: leaderboard.is_combined,
  });
...
export function startUpdateLeaderboardsJob(client: Client): () => void {
  ...
  const runOnce = async (): Promise<void> => {
    ...
    try {
      ...
      logInfo('✅ Leaderboards updated successfully');
    } catch (err: unknown) {
      logError('❌ Failed to update leaderboards:', err);
    }
  };
